...
GPU_PROFILES = {
    ...
    "a100": {"gpu_type": "NVIDIA A100 80GB PCIe", "vram_gb": 80, "disk_gb": 260,
             "label": "A100 80GB — видео A14B, с оффлоадом"},
    # 96 GB и выше — единственный способ убрать оффлоад на A14B: два эксперта в bf16 занимают
    # 56 GB, плюс T5 около 9 GB, и на 80 GB не остаётся памяти под внимание при 81 кадре 720p.
    # Стоят дороже A100, поэтому окупаются только если ускорение больше разницы в цене:
    # PRO 6000 нужно 1.4x, H200 — 3.0x. Проверяется одним роликом.
    "pro6000": {"gpu_type": "NVIDIA RTX PRO 6000 Blackwell Server Edition", "vram_gb": 96,
                "disk_gb": 260, "label": "RTX PRO 6000 96GB — A14B без оффлоада"},
    "h200": {"gpu_type": "NVIDIA H200", "vram_gb": 141, "disk_gb": 300,
             "label": "H200 141GB — A14B с запасом"},
}
